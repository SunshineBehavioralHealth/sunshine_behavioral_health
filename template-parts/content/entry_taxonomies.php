...
<div class="entry-taxonomies">
	...
	foreach ( $taxonomies as $taxonomy ) { // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
		...
		$separator = _x( ', ', 'list item separator', 'sunshine_behavioral_health' );
		...
		switch ( $taxonomy->name ) {
			case 'category':
				...
				$placeholder_text = __( ' %s', 'sunshine_behavioral_health' );
				break;
			case 'post_tag':
				$class            = 'tag-links term-links';
				$list             = get_the_tag_list( '', esc_html( $separator ), '', $post->ID );
				/* translators: %s: list of taxonomy terms */
				$placeholder_text = __( 'Tagged %s', 'sunshine_behavioral_health' );
				break;
			default:
				...
				$placeholder_text = sprintf(
					...
					__( '%s:', 'sunshine_behavioral_health' ),
